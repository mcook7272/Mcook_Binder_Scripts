...
Author:				M. Cook
JIRA:				CCDA-3659
Study PI:			Dr. Nestoras Mathioudakis
Study Title:		Effectiveness of Fully-Automated Digital vs. Human Coach-Based Diabetes Prevention Programs
Date:				10/30/2020
Contact:			Dr. Mohammed Abusamaan
Extract purpose:	Provide query for MyChart recruitment
Current IRB status:	IRB00265873
Funding available:	Please give us an IO
Extract frequency:	Run as needed as a MyChart message
Data Source:		Epic
Extract Structure:	SQL query delivered to Epic Research team to create a Reporting Workbench report that the recruitment team can run as needed.
Data Delivered To:	Epic Research team
...
Exclusion criteria:	Patients with the following criteria will be excluded from the extract results:
						...
						16.	Has received another recruitment email from Hopkins in the last month.  
Revision History:
Date            Author          JIRA            Comment
08-27-21		G.Li			2309			changing requirement.
01-27-22		M.Mitra			2309			change lab lookback period to 6 months
04-13-22		M.Cook			3659			Updating inclusion criteria 3 per spec
...
IF EXISTS #ccda2309_cohort
   SELECT pat.PAT_ID
   INTO #ccda2309_cohort
   FROM PATIENT pat
   INNER JOIN PATIENT_4 pat4 ON pat.pat_id = pat4.pat_id
   INNER JOIN PATIENT_MYC mychart ON mychart.pat_id = pat.pat_id
   INNER JOIN MYC_PATIENT myc ON pat.PAT_ID = myc.PAT_ID
   INNER JOIN COMMUNICATION_PREFERENCES cp ON cp.PREFERENCES_ID = pat4.PREFERENCES_ID
   INNER JOIN COMM_PREFERENCES_APRV aprv ON cp.PREFERENCES_ID = aprv.PREFERENCES_ID
      AND cp.LINE = aprv.GROUP_LINE
   LEFT JOIN PATIENT_FYI_FLAGS fyif ON fyif.PATIENT_ID = pat.PAT_ID
      AND fyif.PAT_FLAG_TYPE_C = '1092'
   WHERE mychart.MYCHART_STATUS_C = '1' -- Patients with an active MyChart account
      ...
      AND fyif.PATIENT_ID IS NULL 
      -- Exclude patients who have opted out of being contacted for research recruitment
      AND cp.COMMUNICATION_CONCEPT_ID = '28102' 
      -- Include patients who approved bulk communication as a communication concept.
      AND aprv.APRV_MEDIA_C = '2' 
      -- Include patients where MyChart is the approved communication media for a communication concept.
      AND FLOOR(DATEDIFF(dd, BIRTH_DATE, @_Today) / 365.24256) BETWEEN 18
   ...
   FROM (
      SELECT PAT_ID
      FROM PAT_ENC e
      INNER JOIN CLARITY_DEP d ON e.EFFECTIVE_DEPT_ID = d.DEPARTMENT_ID
      WHERE BMI >= 25
      ...
      FROM PAT_ENC e
      INNER JOIN CLARITY_DEP d ON e.EFFECTIVE_DEPT_ID = d.DEPARTMENT_ID
      INNER JOIN PATIENT_RACE p ON e.PAT_ID = p.PAT_ID
   ...
   FROM (
      ...
      INNER JOIN order_proc orp ON res.ORDER_PROC_ID = orp.ORDER_PROC_ID
      INNER JOIN pat_enc enc ON orp.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN clarity_component cc ON res.COMPONENT_ID = cc.COMPONENT_ID
	  INNER JOIN CLARITY_DEP DEP ON DEP.DEPARTMENT_ID = enc.DEPARTMENT_ID
      ...
      FROM order_results res
      INNER JOIN order_proc orp ON res.ORDER_PROC_ID = orp.ORDER_PROC_ID
      INNER JOIN order_proc_2 orp2 ON orp.order_proc_id = orp2.order_proc_id
      INNER JOIN pat_enc enc ON orp.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN clarity_component cc ON res.COMPONENT_ID = cc.COMPONENT_ID
      INNER JOIN CLARITY_DEP DEP ON DEP.DEPARTMENT_ID = enc.DEPARTMENT_ID
      WHERE ISNULL(DEP.SERV_AREA_ID, 11) = 11
      ...
      FROM (
         ...
         FROM CLARITY.DBO.PATIENT PAT
         INNER JOIN CLARITY.DBO.PAT_ENC PE ON PAT.PAT_ID = PE.PAT_ID
         INNER JOIN CLARITY.DBO.HSP_ACCT_DX_LIST DXL WITH (NOLOCK) ON PE.HSP_ACCOUNT_ID = DXL
            .HSP_ACCOUNT_ID
...
--1.	Aortic stenosis (I35.0) active on problem list or encounter diagnosis within last 3 years 
DELETE
FROM #ccda2309_cohort
WHERE PAT_ID IN (
      SELECT cht.PAT_ID
      FROM #ccda2309_cohort cht
      INNER JOIN PROBLEM_LIST prb ON cht.PAT_ID = prb.PAT_ID
      INNER JOIN EDG_CURRENT_ICD10 icd ON prb.DX_ID = icd.DX_ID
      ...
      SELECT enc.PAT_ID
      FROM #ccda2309_cohort cht
      INNER JOIN PAT_ENC enc ON cht.PAT_ID = enc.PAT_ID
      INNER JOIN PAT_ENC_DX pex ON pex.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN EDG_CURRENT_ICD10 icd ON pex.DX_ID = icd.DX_ID
      INNER JOIN CLARITY_DEP DEP ON DEP.DEPARTMENT_ID = enc.DEPARTMENT_ID
      WHERE ISNULL(DEP.SERV_AREA_ID, 11) = 11
         AND CODE LIKE 'I35.0%'
...
WHERE PAT_ID IN (
      SELECT cht.PAT_ID
      FROM #ccda2309_cohort cht
      INNER JOIN F_IP_HSP_ADMISSION adm ON cht.PAT_ID = adm.PAT_ID
      INNER JOIN PAT_ENC_HSP hsp ON adm.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
      --JOIN HSP_ACCT_ADMIT_DX dx on hsp.HSP_ACCOUNT_ID = dx.HSP_ACCOUNT_ID
      INNER JOIN HSP_ACCT_DX_LIST dx ON hsp.HSP_ACCOUNT_ID = dx.HSP_ACCOUNT_ID
      INNER JOIN EDG_CURRENT_ICD10 icd ON dx.DX_ID = icd.DX_ID
...
WHERE PAT_ID IN (
      ...
      FROM #ccda2309_cohort cht
      INNER JOIN PAT_ENC enc ON cht.PAT_ID = enc.PAT_ID
      INNER JOIN ORDER_MED orm ON orm.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN CLARITY_MEDICATION med ON orm.MEDICATION_ID = med.MEDICATION_ID
      INNER JOIN CLARITY_DEP DEP ON DEP.DEPARTMENT_ID = enc.DEPARTMENT_ID
      WHERE ISNULL(DEP.SERV_AREA_ID, 11) = 11
...
WHERE PAT_ID IN (
      ...
      INNER JOIN ORDER_MED orm ON orm.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN CLARITY_MEDICATION med ON orm.MEDICATION_ID = med.MEDICATION_ID
      INNER JOIN ZC_PHARM_CLASS zpc ON med.PHARM_CLASS_C = zpc.PHARM_CLASS_C
      INNER JOIN CLARITY_DEP DEP ON DEP.DEPARTMENT_ID = enc.DEPARTMENT_ID
      WHERE ISNULL(DEP.SERV_AREA_ID, 11) = 11
...
WHERE PAT_ID IN (
      ...
      FROM #ccda2309_cohort cht
      INNER JOIN PAT_ENC enc ON cht.PAT_ID = enc.PAT_ID
      INNER JOIN PAT_ENC_DX pex ON enc.PAT_ENC_CSN_ID = pex.PAT_ENC_CSN_ID
...
WHERE PAT_ID IN (
      ...
      FROM #ccda2309_cohort cht
      INNER JOIN pat_enc enc ON cht.PAT_ID = enc.PAT_ID
      INNER JOIN order_proc orp ON orp.PAT_ENC_CSN_ID = enc.PAT_ENC_CSN_ID
      INNER JOIN order_results res ON res.ORDER_PROC_ID = orp.ORDER_PROC_ID
...
WHERE PAT_ID IN (
      SELECT cht.PAT_ID
      FROM #ccda2309_cohort cht
      INNER JOIN MEDICAL_HX mhx ON cht.PAT_ID = mhx.PAT_ID
      INNER JOIN EDG_CURRENT_ICD10 icd ON mhx.DX_ID = icd.DX_ID

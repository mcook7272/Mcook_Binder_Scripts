...
Will need to designate the name of your own OMOP database, and schema you want dbo to be loaded into.
*/
USE PMAP_Analytics; 

--Drop all tables
DROP TABLE IF EXISTS dbo.CURE_ID_Cohort; --Change schema as appropriate
...
CREATE TABLE [dbo].[CURE_ID_Cohort] (
...
--First identify patients (inpatient and outpatient) with covid positive lab dbo
SELECT DISTINCT person_id
   ,measurement_date
INTO #covid_lab_pos
from JHM_OMOP_NoID.dbo.MEASUREMENT
WHERE measurement_concept_id IN (
      SELECT concept_id
      from JHM_OMOP_NoID.dbo.CONCEPT
      ...
      SELECT c.concept_id
      from JHM_OMOP_NoID.dbo.CONCEPT c
      INNER join JHM_OMOP_NoID.dbo.CONCEPT_ANCESTOR ca ON c.concept_id = ca.descendant_concept_id
   ...
   AND (
      -- The value_as_concept field is where we store standardized qualitative dbo
...
INTO #inpat_intermed
FROM JHM_OMOP_NoID.dbo.visit_occurrence v
...
INTO #Vis_Occ
FROM #los_max v
INNER JOIN JHM_OMOP_NoID.dbo.person p ON v.person_id = p.person_id
LEFT JOIN JHM_OMOP_NoID.dbo.death d ON v.person_id = d.person_id;

--Creates cohort by adding on birth date and death date
INSERT INTO [dbo].[CURE_ID_Cohort] --Change schema if not using dbo
   (

...
/*1. select patients with 3 plus anti-VEGF medication within 121 days from first medication */  -- person_id 1056 : count 7
...
--drop table #drug_exposure_121
select #3plus_count.person_id, med_count,#3plus_count.drug_concept_id, 
		d_startdate,drug_exposure_start_date,DATEDIFF(day,d_startdate,drug_exposure_start_date) as days_from_first_med
into #drug_exposure_121
from #3plus_count
...
select * from #drug_exposure_121 order by person_id

--drop table #drug_exposure_within121
select de.person_id, med_count as all_med_count,med_count_within121 as med_count_within121, d_startdate, drug_exposure_start_date,days_from_first_med
into #drug_exposure_within121
from #drug_exposure_121 de
inner join (
			select person_id, count(person_id) as med_count_within121
			from #drug_exposure_121
			group by person_id) as med_count

on de.person_id = med_count.person_id
order by de.person_id
select * from  #drug_exposure_within121

/** patient injection records from first medication within 121 days **/
--drop table #injection_within121
select mi.person_id, med_concept_id, d_startdate, procedure_concept_id, p_date,days_from_first_med, injection_count_within121
into #injection_within121
from #med_injection_days mi
inner join (select  person_id,  count(person_id) as injection_count_within121
			from #med_injection_days
			where days_from_first_med<121
			group by person_id )  as injection_count 
on mi.person_id = injection_count.person_id
where mi.days_from_first_med < 121 
order by mi.person_id , days_from_first_med


select de121.person_id, med_count_within121,injection_count_within121 
into #med_injection_compare
from 
	(select person_id, med_count_within121 
	from #drug_exposure_within121
	group by person_id, med_count_within121) as  de121 
inner join 
	(select person_id, injection_count_within121 
	from #injection_within121
	group by person_id, injection_count_within121) as  injection121
on de121.person_id = injection121.person_id 
order by de121.person_id 

/**tables to export **/ 

select * from #drug_exposure_within121 order by person_id 
select * from #injection_within121 order by person_id
select * from #med_injection_compare order by person_id 
...
union all
select distinct p.person_id, s.sourcekey as osler_id, 'med_injection_compare' "name"
from #med_injection_compare  p
join stage.SourceIDMapsPerson s on s.id = p.person_id
--WHERE p.p_count < p.medication_count
union all

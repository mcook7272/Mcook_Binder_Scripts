...
SELECT DISTINCT pat_id
   ...
   ,delta_sum
   ,ROW_NUMBER() OVER (
      PARTITION BY pat_mrn_id ORDER BY combined_percentage_change
      ) AS RN
INTO #temp
FROM (
   SELECT DISTINCT pat_id
      ...
      ,delta_sum
      ,ROW_NUMBER() OVER (
         PARTITION BY control_mrn_id ORDER BY combined_percentage_change
         ) AS pat_num
...
SELECT DISTINCT pat_mrn_id
   ...
   ,CASE 
      WHEN RN < 5
         THEN 'YES'
      ELSE 'NO'
      END four_lowest_combined_percentage_change
   ,delta
   ,delta_los
   ,abs_delta_los
   ,delta_age

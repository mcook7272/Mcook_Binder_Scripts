...
/****** Object:  StoredProcedure [dbo].[CCDA2046_Cox_NoHyphens]    Script Date: 6/29/2022 9:02:17 AM ******/
SET ANSI_NULLS ON
GO
...
CTE_preg AS (
SELECT DISTINCT t.osler_id, max(t.Last_pregnancy_date) AS Last_pregnancy_date
FROM (
   SELECT ep.osler_id, ep.END_DATE AS Last_pregnancy_date
   FROM [PMAP_Analytics].[dbo].[custom_clarity_episode] ep
   WHERE (
      ep.SUM_BLK_TYPE_ID = '14' --14=delivery episode
      AND ep.birth_date < ep.START_DATE
      ) --don't get id of newborn
   OR ep.SUM_BLK_TYPE_ID = '2' --2 = pregnancy episode
   UNION
   SELECT DISTINCT osler_id, contact_date AS pregnancy_date
   FROM COVID_Projection.dbo.derived_outpatient_encounters enc
   WHERE department_id = '110302200'
      AND ENC_TYPE_C IN ('1003', '101', '2531')
      AND APPT_PRC_ID NOT IN ('1001', '321072', '321069')
      ---*** added 1/6/22* to exclude non-pregnant patients**
      AND coalesce(APPT_STATUS_C, 0) NOT IN (3, 4, 5) 
      -- no canceled, no shows, or left without seen
      AND contact_date <= CURRENT_TIMESTAMP
   UNION
   SELECT DISTINCT osler_id, last_date
   FROM COVID_Projection.dbo.derived_emr_diagnosis_info
   WHERE (
         icd10list LIKE '%O[0-9][0-9].%'
         OR icd10list LIKE '%Z34.%'
         OR icd10list LIKE '%Z35.%'
         OR icd10list LIKE '%Z36.[0-8]%'
         OR icd10list LIKE '%Z33.1%'
         OR icd10list LIKE '%Z64.0%'
         )
   UNION
   SELECT DISTINCT osler_id, proc_date
   FROM COVID_Projection.dbo.derived_profee_billing_px
   WHERE (
         code LIKE '59%'
         OR code IN ('0500F', '0501F', '0502F', '0503F', '76801', '76828', '59025')
         )
   ) t
GROUP BY t.osler_id
)
...
SELECT DISTINCT
	...
	,CASE 
		WHEN PT.gender = 'Male' THEN 'N/A' 
		WHEN preg.osler_id IS NOT NULL THEN 'Y' 
		ELSE 'N' 
		END AS 'Pregnancy_Status'
	,preg.Last_pregnancy_date
	--,who.rn
...
FROM 
	...
	LEFT JOIN CTE_PCR pcr ON ENROLL.osler_id = pcr.osler_id
	LEFT JOIN CTE_Preg preg ON ENROLL.osler_id = preg.osler_id

...
Date:			2022-02-09
JIRA:			[CCDA-4920]
Description:	Evaluate the Safety, Tolerability, and Effectiveness of CIN-107 in Patients with Primary Aldosteronism

Inclusion:		
1.	Age: >=18 years old as of the query runs.
2.	Location: Johns Hopkins Bayview Medical Center, Johns Hopkins Hospital
3.	From 01 January 2019: Patients with a diagnosis of ICD10: E26.9 or E26.09. Will look for diagnosis in problem list (active), and encounter/billing diagnosis.
...
FROM 
		...
		AND PL.NOTED_DATE >= '2019-01-01' --From 01 January 2019
		UNION
		...
		WHERE DX.CONTACT_DATE >= '2019-01-01' --From 01 January 2019
		UNION
		...
		WHERE ARPB.SERVICE_DATE >= '2019-01-01' --From 01 January 2019
		UNION
		SELECT DISTINCT PAT.PAT_ID  
		FROM CLARITY..PATIENT PAT
		...
		INNER JOIN #inc_dx I ON I.DX_ID = EDG.DX_ID
		WHERE PE.CONTACT_DATE >= '2019-01-01' --From 01 January 2019
		) DX
...
WHERE
		...
		OR DEP.DEPARTMENT_ID LIKE '1102%') --JHH or BMC
		AND ISNULL(PE.APPT_STATUS_C,2) = 2 
		AND FLOOR(DATEDIFF(DD,P.BIRTH_DATE,GETDATE())/365.25) >= 18

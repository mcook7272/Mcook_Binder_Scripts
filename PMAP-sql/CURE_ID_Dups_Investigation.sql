...
--Make all flags
--Get rid of CTE's, make (keep) temp tables
--Apply joins sequentially
--End up with only one vis per pstient
--Revisit deid to make sure no dupes; clean up too
--Finds closest encounter to first positive SARS-COV-2 test (for patients hospitalized more than once)
SELECT person_id
...
--into #closest_vis
from #inpat i
join inpat_closest_vis c on i.person_id = c.person_id
...
#inpat
--visit_occurrence
where person_id = 1333580

#inpat
where person_id = 945862

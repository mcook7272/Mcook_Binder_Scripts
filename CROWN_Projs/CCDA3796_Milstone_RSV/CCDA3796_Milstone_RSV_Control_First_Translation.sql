...
AS (
   ...
   FROM (
      ...
      INNER JOIN clarity..zc_patient_race zc_patient_race ON patient_race.patient_race_c = zc_patient_race
         .patient_race_c
...
AS (
   ...
   FROM (
      ...
      LEFT JOIN clarity..PATIENT pat ON ethnic_background.PAT_ID = pat.PAT_ID
...
SELECT DISTINCT p.pat_id
   ...
   ,pe.pat_enc_csn_id
   ,WT.MEAS_VALUE "Weight_In_Ounces"
   ,HT.MEAS_VALUE "Height_In_Inches"
   ,BMI.MEAS_VALUE "BMI"
   ,pe.contact_date
   ,pe.hosp_admsn_time
...
INNER JOIN clarity.dbo.pat_enc pe ON pe.pat_enc_csn_id = adt.pat_enc_csn_id
...
SELECT DISTINCT c01.pat_id
   ...
   ,(
      ...
      abs(100 - (
            round((
                  (
                     round(DATEDIFF(day, p.birth_date, peh.hosp_admsn_time), 0) / c01.
                     age_in_days_resultdate
                     ) * 100
                  ), 2)
            ))
